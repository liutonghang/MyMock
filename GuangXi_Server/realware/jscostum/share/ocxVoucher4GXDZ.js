...
function storeLoad(store,dzFields,jsonMap){
		store.load({
		method : 'post',
		params : {
			start : 0,
			pageSize : 200,
			filedNames : JSON.stringify(dzFields),
			jsonMap : jsonMap
		}
//		,
//		callback:function(rcrd){
//			if(rcrd.length>0){
//				Ext.getCmp("dzpanel").getSlectionMode().selectRow(0);
//				Ext.getCmp("dzpanel").getView().focusRow(0);
//			}
//		}
	});
}
function lookOCX(records, idName, vt_code,check_result) {
	if (records.length == 0) {
...
//	      }
//	}
	//ztl  2016年9月20日11:32:29  重新建立新的store，以便对账查询时，显示凭证号、对账单号金额等信息。
	var urlpath = "/realware/loadCheckMain.do";;
	//注释：对账主单主键，对账凭证号，对账单号，业务类型ID，单据ID，总金额，凭证类型     
	var dzFields = ["check_voucher_id","voucher_no","voucher_check_no","biz_type_id", "bill_type_id", "all_amt","vt_code","last_ver"];
	var jsonMap = '[{\'voucher_check_id\':[\'=\',' +records[0].data.voucher_check_id + ']}]';
	var storeCheckMain = getStore(urlpath, dzFields);
	storeLoad(storeCheckMain,dzFields,jsonMap);
	
//	// 列表
//	var gridStore = Ext.create('Ext.data.Store', {
//				fields : [{
//							name : 'check_voucher_id'
//						}, {
//							name : 'voucher_no'
//						}, {
//							name : 'voucher_check_no'
//						}, {
//							name : 'biz_type_id'
//						}, {
//							name : 'bill_type_id'
...
//							showOCX(selected[0].get(idName), selected[0].get('bill_type_id'), vt_code);
						showOCX(selected[0].get('check_voucher_id'), selected[0].get('bill_type_id'), vt_code);
						}
					}
				}]
			}, {
...
//	showOCX(selected[0].get(idName), records[0].get('bill_type_id'), vt_code);
//	showOCX(selected[0].get('check_voucher_id'), selected[0].get('bill_type_id'), vt_code);
	w1.show();
}
...
function initPrintViewByXml(records,idName,reports){
	...
	Ext.Array.each(records, function(model) {
		voucherNos.push(model.get(idName.replace('_id', "_no")))
	});
