...
var fileds = ["pay_voucher_code", "vou_date", "pay_amount", "payee_account_no",
		...
		"last_ver", "return_reason" , "create_user_code"];

/**
 * 列名
 */
var header = "退票原因|return_reason|150,凭证号|pay_voucher_code|140,凭证日期|vou_date|100,支付金额|pay_amount|100,柜员号|create_user_code,收款人账号|payee_account_no,"
...
function selectState() {
	...
	if ("004" == taskState) {
		...
		Ext.getCmp("hungaccount").disable(false);
		...
		Ext.getCmp("repeatMoney").disable(false);
		Ext.getCmp("hungaccount").enable(false);
...
function batchReqMoney() {
	...
	if (records.length == 0) {
			...
			return;
		}else if(records.length>0 && vtCode == '8202' && records.length !=1){
				Ext.Msg.alert("系统提示", "授权支付无法批量请款！");
				return;
		}
...
function checkHungUpAcoount() {
	...
	Ext.Ajax.request({
				method : 'POST',
				timeout : 180000, // 设置为3分钟
				url : '/realware/checkHungUpAcoount.do',
				params : {
					// 单据类型id
					billTypeId : records[0].get("bill_type_id"),
					billIds : Ext.encode(reqIds),
