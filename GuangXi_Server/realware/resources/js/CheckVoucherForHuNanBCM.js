...
var fileds = ["payee_account_bank_no", "pay_voucher_code", "vou_date",
		...
		"pb_set_mode_name","audit_remark","create_user_code","remark"];

/**
 * 列名
 */
var header = "退票原因|return_reason|150,退回原因|audit_remark|150,收款行行号|payee_account_bank_no|100,银行结算方式名称|pb_set_mode_name,银行结算方式名称|pb_set_mode_name|140,行号补录|do1|100|addBankno,凭证号|pay_voucher_code|140,凭证日期|vou_date|100,支付金额|pay_amount|100,柜员号|create_user_code,收款人账号|payee_account_no,"
		...
		+ "支付方式名称|pay_type_name,结算方式编码|set_mode_code,结算方式名称|set_mode_name,银行结算方式编码|pb_set_mode_code";
...
function addBankno(grid, rowIndex, colIndex, node, e, record, rowEl) {
	...
	bankStore.load(function(records, operation, success) {
		...
		if (b) {
					...
					items : [new Ext.FormPanel({
						...
						buttons : [{
							...
							handler : function() {
								...
								grid.getStore().data.items[rowIndex1].set(
										"remark", Ext
												.getCmp('remark').getValue());
								//控制附言确定后重置
								if(!Ext.isEmpty(Ext.getCmp('remark'))) {
			...
			var tempRemark = grid.getStore().data.items[rowIndex1]
					.get("remark");
					
			if (tempRemark == null || tempRemark == '') {
				
			} else {
				Ext.getCmp("remark").setValue(tempRemark);
			}
...
//				}
				...
				jsonMap += "{\"id\":\"" + model.get("pay_voucher_id")+"\",\"bankNo\":\"" + payeeAcctBankno+"\",\"remark\":\"" + remark+"\",\"setModeCode\":\"" +  model.get("pb_set_mode_code") +"\",\"setModeName\":\"" + model.get("pb_set_mode_name") + "\"},";
			});
