...
var fileds = ["pay_voucher_code", "vou_date", "pay_amount","refund_type", "payee_account_no",
		...
		"pay_summary_name", "task_id", "pay_voucher_id", "bill_type_id","last_ver","remark","ori_voucher_id"];
...
function voucherInput() {
	if (inputWin == null) {
		...
		gridOrivoucher.setHeight(h * 0.4);
		gridOriRequest.setHeight(h * 0.21);
			...
			items : [Ext.widget('form', {
				...
				items : [{
							...
							items : [{
										...
										text : '&nbsp;查看&nbsp;',
										handler : function() {
										  lookOCX(gridOrivoucher.getSelectionModel().getSelection(),"pay_voucher_id");					
										}
									}, {
										id : 'summaryName',
										fieldLabel : '&nbsp;&nbsp;&nbsp;&nbsp;资金用途',
...
//			if (admdiv == null || admdiv == "")
//				return;
//			beforeload(Ext.getCmp("oriquery"), options, Ext.encode(fileds));
//		});	
	}
...
function refleshQuery(g) {
	...
	if( Ext.isEmpty(startDate) && Ext.isEmpty(endDate) && Ext.isEmpty(payeeAcctNo)
			&& Ext.isEmpty(summaryName) && Ext.isEmpty(aAmount) && Ext.isEmpty(eAmount)
			&& Ext.isEmpty(payAcctNo) && Ext.isEmpty(voucherNo)){
