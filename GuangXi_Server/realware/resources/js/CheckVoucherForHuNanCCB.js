...
var fileds = ["payee_account_bank_no", "pay_voucher_code", "vou_date",
		...
		"pb_set_mode_name","audit_remark","batchreq_status"];

/**
 * 列名
 */
var header = "退票原因|return_reason|150,退回原因|audit_remark|150,收款行行号|payee_account_bank_no|100,银行结算方式名称|pb_set_mode_name|140,行号补录|do1|100|addBankno,凭证号|pay_voucher_code|140,凭证日期|vou_date|100,支付金额|pay_amount|100,收款人账号|payee_account_no,"
		...
		+ "支付方式名称|pay_type_name,结算方式编码|set_mode_code,结算方式名称|set_mode_name,银行结算方式编码|pb_set_mode_code,银行结算方式名称|pb_set_mode_name";
 ...
 * @return {TypeName} 
 */
var localBankStore = Ext.create('Ext.data.Store', {
	fields : [ 'bank_name', 'bank_no' ],
	data : [ {
		"bank_name" : "安徽",
		"bank_no" : "53272"
	}, {
...
function addBankno(grid, rowIndex, colIndex, node, e, record, rowEl) {
	var b = true;
	rowIndex1 = rowIndex;
	var bankNo = record.data['payee_account_bank_no'];
	if(bankNo.length==5){
	 Ext.create('Ext.window.Window', {
			...
			items : [new Ext.FormPanel({
				...
				buttons : [{
					text : '确定',
					handler : function() {
						var records = Ext.getCmp('localGridBankno').getSelectionModel().getSelection();
						if (records.length == 0)
							return;
	 	...
	 	Ext.getCmp("localGridBankno").getSelectionModel().select(0);
	}else{
