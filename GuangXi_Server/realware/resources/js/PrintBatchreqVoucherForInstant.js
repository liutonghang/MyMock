...
var filed001 = [ "bill_no", "batch_no", "pay_amt", "batchreq_date", "pay_account_no",
...
var header001="单号|bill_no|130,请款单号|batch_no|130,总支付金额|pay_amt|100,状态|batchreq_cstatus|140,请款日期|batchreq_date,支付总笔数|pay_allnum,零余额账号|payee_account_no|130,零余额账户名称|payee_account_name|140,零余额银行|payee_account_bank|140,"
...
function selectState() {
	var taskState = Ext.getCmp('taskState').getValue();
	if ("001" == taskState) {
		if( createReqMethod != undefined && "createDaily" == createReqMethod ){
			Ext.getCmp('create').enable(false);
			Ext.getCmp("createBatchReq").setVisible(false);
		}else if( createReqMethod != undefined && "createInstant" == createReqMethod ){
			Ext.getCmp('createBatchReq').enable(false);
		...
		if( createReqMethod != undefined && "createDaily" == createReqMethod ){
			...
			Ext.getCmp('createBatchReq').disable(false);
			Ext.getCmp("create").setVisible(false);
		}
	}
	refreshData();
...
/***
 * 即时生成批量请款信息，可以每天生成多笔，分组生成
 */
function createBatchReqVoucher(){
	var myMask = new Ext.LoadMask(Ext.getBody(), {
			msg : '后台正在处理中，请稍后....',
			removeMask : true // 完成后移除
		});
					...
					buttons : [{
						...
						handler : function() {
							if (this.up('form').getForm().isValid()) {
								...
								myMask.show();
								Ext.Ajax.request({
									url : '/realware/saveBatchreqVoucherInstant.do',
									method : 'POST',
									timeout : 180000, // 设置为3分钟
									params : {
										batchreq_date : batchreqDate,
