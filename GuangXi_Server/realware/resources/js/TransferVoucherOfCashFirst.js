...
var fileds = [ "pay_voucher_code", "vou_date", "pay_amount",
		...
		"pay_mgr_name","business_type","sup_dep_name","sup_dep_code" ];
...
function query() {
	...
	Ext.Ajax.request( {
		...
		params : {
			...
			jsonMap : "[{\"admdiv_code\":[\"=\",\"" + admdivCode
					+ "\"],\"pay_voucher_code\":[\"=\",\"" + billNo
					+ "\"],\"pay_amount\":[\"=\"," + oriPayAmt + "],\"account_type_right\":[\"=\",\""+ account_type_right +"\"],\"task_id\":[\"=\",0]}]",
			start : 0,
			page : 1,
			limit : 1
		},
		success : function(response, options) {
			...
			if (dto.pageCount == 0) {
				Ext.Msg.alert("系统提示", "根据凭证号和金额没有查询到授权支付特殊数据,请查看是否已退回！");
				return;
			}
			objDTO = dto.root[0];
			...
			Ext.getCmp("payConfrim").enable(false);
			if (objDTO.set_mode_code == cashMark) {
//				Ext.getCmp("qingkuan").disable(false);
			} else {
//				Ext.getCmp("qingkuan").enable(false);
			}
			setValue(objDTO);
		},
...
function createCashWindow(cashMark)
{
		if (cashWin == null) {
		var h = document.documentElement.scrollHeight-80;
...
//										Ext.Msg.alert("系统提示","非授权支付特殊业务无法执行此操作！");
//										return;
//									}
									var ids=[];
									var lastVers = [];
...
//									}
									if(check()){
//										if(objDTO.set_mode_code !=cashMark && objDTO.batchreq_status!=1){
//											Ext.Msg.alert("系统提示","请先请款后再进行支付确认！");
//											return;
//										}
										transVoucher(2);
										cashWin.hide();
...
//	Ext.getCmp('qingkuan').disable(false);
	Ext.getCmp('payConfrim').disable(false);
	Ext.getCmp('rengongCfrim').disable(false);
...
function check() {
	...
	if(leibei =="1"){
		...
		if (oriPayAmt < Amt || Amt == 0 ||  Amt == null || Amt <0) {
...
function refreshData() {
	gridPanel1.getStore().load({
			 callback: function(records, operation, success) {
					if(records.length>0 && Ext.getCmp('taskState').getValue()!="008"){
						for(var i=0;i<records.length;i++){
							var rec = gridPanel1.getStore().getAt(i);
							rec.set('pay_amount', 0.00);
							rec.commit();
						}
					}
			 }
