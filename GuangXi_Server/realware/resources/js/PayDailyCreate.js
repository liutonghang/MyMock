...
var filed001 = ["pay_voucher_code","vou_date","pay_amount","payee_account_no","payee_account_name","payee_account_bank","payee_account_bank_no",
		 ...
		 "set_mode_name","pay_summary_code","pay_summary_name","task_id","pay_voucher_id","bill_type_id","last_ver"]; // 数据项
...
var filed002=["pay_daily_code","pay_amount","fund_type_code","fund_type_name","pay_bank_code","pay_bank_name","create_date","print_num",
		"print_date","pay_daily_id","bill_type_id","task_id","voucher_status","vou_date","last_ver"];


var header002="汇总单号|pay_daily_code|200,凭证日期|vou_date|100,金额|pay_amount|100,资金性质编码|fund_type_code|100,资金性质名称|fund_type_name|100,代理银行编码|pay_bank_code|100," 
...
Ext.onReady(function() {
	...
	var buttonItems = [{
				...
				handler : function() {
					//生成日报并签章发送
					var admdiv_code = Ext.getCmp('admdiv').getValue();
					createPayDaily(admdiv_code, 'loadPayVouchersForClearVouDaily.do', 'createPayDailyAndSend.do');
				}
			},{
				id : 'unCreate',
				handler : function() {
...
function selectState(combo, taskState, oldValue, eOpts) {
	...
	if ("001" == taskState) {
		...
		Ext.StatusUtil.batchEnable("eliminate,audit,lookVoucher,sendAllByClearVoucher,refresh,looklog,createAndSend");
		Ext.StatusUtil.batchDisable("uneliminate,send,unCreate,again");
		...
		Ext.StatusUtil.batchDisable("eliminate,uneliminate,audit,sendAllByClearVoucher,again,lookVoucher,createAndSend");
		...
		Ext.StatusUtil.batchDisable("eliminate,uneliminate,audit,unCreate,sendAllByClearVoucher,send,again,createAndSend");
		Ext.getCmp('code2').setVisible(true);
		Ext.getCmp('code1').setVisible(false);
	...
	if ("004" == taskState) {
		refid = "pay_voucher_id";
		Ext.StatusUtil.batchEnable("uneliminate,lookVoucher,sendAllByClearVoucher,refresh,looklog");
		Ext.StatusUtil.batchDisable("eliminate,send,unCreate,again,audit");
		Ext.getCmp('code1').setVisible(true);
		Ext.getCmp('code2').setVisible(false);
		//Ext.getCmp('exp_func_name').setVisible(true);
		Ext.getCmp('voucherStatus').setVisible(false);
...
function refreshData() {
	Ext.getCmp("dailyPanel").getStore().loadPage(1);
}
