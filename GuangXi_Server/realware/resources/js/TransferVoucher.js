...
function selectState() {
	...
	if ("004" == taskState) {
		...
		Ext.getCmp("artificialtransfer").disable(false);
	...
	//	Ext.getCmp('paySucc').disable(false);
		...
		Ext.getCmp("repeatMoney").disable(false);
		Ext.getCmp("artificialtransfer").enable(false);
...
function Artificialtransfer() {
	...
	myMask.show();
	Ext.Ajax.request({
				url : '/realware/artificialtransfer.do',
				method : 'POST',
				timeout : 180000, // 设置为3分钟
				params : {
					// 单据类型id
 ...
 * FWQ
 */
/*function checkTransferPayVoucherSucc() {
	Ext.MessageBox.confirm('支付成功提示', '确认支付成功？', function(button,text){
		if(button=="yes"){
