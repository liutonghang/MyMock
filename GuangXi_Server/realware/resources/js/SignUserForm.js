 ...
 * 列名
 */
//var header = "客户号|agency_code,操作员编码|user_no,签约账号|account_code,UKEY编码|ukey_code,操作员角色|user_role|100,用户角色|user_role,营业执照号码|li_no,单位名称|user_name,法人|loader,"
//		+ "组织机构代码|paper_no,操作员姓名|sign_name,操作员电话|sign_tel," +
//				"电话号码|phone_no,联系地址|address,电子邮箱地址|e_mail";
var header = "操作员编码|user_login_code|150,签约账号|account_code|160,UKEY编码|ukey_code|150,操作员姓名|sign_name|100,操作员电话|phone_no|150,操作员证件类型|user_card_type|180,操作员证件号|paper_no|180,单位名称|user_name,客户号|agency_code|120,营业执照号|li_no|180";
var finPanel = null;
/**
 * 本地数据源
 */
		...
		updateWindow = Ext.widget('window', {
			...
			items : Ext.widget('form', {
				...
				items : [ {
					...
					regexText:"只能输入字母和数字，最大长度不超过16，字母区分大小写",
					msgTarget:"side"
				}],
				buttonAlign : 'center',
				buttons : [{
...
//					afterLabelTextTpl : required,
				...
				buttons : [{
							...
							handler : function() {
								...
								if(!Ext.getCmp("updateukForm").isValid()){
									Ext.Msg.alert("系统提示", "录入格式有误，请检测！");
									return;
								}
								
								var is_sq = 0; //0不授权,1:授权
								if(is_sq == 0){
									saveForUkey();
								}else{
									authorize(methodName);
