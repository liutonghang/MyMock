 ...
 *     });
 */
Ext.define('pb.view.common.DateItem', {
    ...
    alias: 'widget.dateitempicker',
    alternateClassName: 'Ext.DateItemPicker',
...
//edit               '<a id="{id}-prevEl" class="{baseCls}-prev {baseCls}-arrow" href="#" role="button" title="{prevText}" hidefocus="on" ></a>',
            '<a id="{id}-prevEl"></a>',   
            '<div class="{baseCls}-month" id="{id}-middleBtnEl">{%this.renderMonthBtn(values, out)%}</div>',
                 // the href attribute is required for the :hover selector to work in IE6/7/quirks
//edit               '<a id="{id}-nextEl" class="{baseCls}-next {baseCls}-arrow" href="#" role="button" title="{nextText}" hidefocus="on" ></a>',
           		'<a id="{id}-nextEl"></a>',   
                '</div>',
...
//edit 选中删除原样式修改为不删除        
    ...
    setValue : function(value){
    	if(value.getMonth()!=this.getValue().getMonth()){
...
//        edit 选中删除原样式修改为不删除cells.removeCls(cls);
        ...
        setCellClass = function(cell, cls){
            ...
            if(cls!=me.prevCls){
            	...
            	if(Ext.isIE6 || Ext.isIE7 || Ext.isIE8 || Ext.isIE9){
					text_ = cell.innerText;
				}
				else {
					text_= cell.textContent;
				} 
            	if(Ext.Array.contains(me.noWorkDays,text_)){ 
            		cls += ' ' + me.selectedCls; 
           		}
            }
