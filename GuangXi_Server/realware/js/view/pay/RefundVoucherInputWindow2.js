 ...
 * @memberOf {TypeName} 
 */
Ext.define('pb.view.pay.RefundVoucherInputWindow2', {
	extend : 'Ext.window.Window',
	alias : 'widget.refundVoucherInputWindow2', //别名
	layout : 'fit',
	modal : true,
	width : '100%',
	height : '100%',
	...
	initComponent : function() {
		...
		//常用查询区
		var queryItems = [];
		if (me.query != 'HuNanABC' && me.query != 'HuNanSS') {
			queryItems = [ {
				name : 'vouNo',
				fieldLabel : '凭证号',
				xtype : 'textfield',
				labelWidth : 70,
				//modify by cyq  2014/12/25  symbol属性值由'LiKE'改为'like',支持模糊查询
				symbol : 'like',
			...
			items : [ {
				...
				items : [ {
					...
					items : [ {
						...
						items : [ {
							...
							id : 'refundSerialNo',
							xtype : 'textfield',
							hidden : me.query == 'HuNanSS' ? false : true,
							fieldLabel : '<font style=\'color:red\'>*</font>退款流水号',
							style : 'margin-left:5px;'
						} ]
					},{
						xtype : 'panel',
				...
				buttons : [ {
					...
					handler : function() {
						...
						// 退款流水号（自助对接用）
						var refundSerialNo = Ext.ComponentQuery.query('textfield[id=refundSerialNo]', me)[0].getValue();
						//是否按单退款
						...
						Ext.MessageBox.show({
				            ...
				            fn: function(btn){
				            	if(btn == 'yes'){
				            		me.fireEvent('saveRefundclick', cbxInput, id,isBill==true?1:0, refundAmt,refundRemark,payType,payFlag,writeoffVouNo,writeoffVouseqno,writeOffuserCode,refundSerialNo);	
				            	}
				            }
						})
					}
