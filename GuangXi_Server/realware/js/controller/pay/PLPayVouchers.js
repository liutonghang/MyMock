 ...
 * @memberOf {TypeName} 
 */
Ext.define('pb.controller.pay.PLPayVouchers', {
	extend : 'Ext.app.Controller',
	// 数据集列表：
	// pay.PayVouchers 支付凭证
	stores : [ 'pay.PayVouchersHunanBOC'],
	//对象模型 pay.PayVoucher 支付凭证
	models : [ 'pay.PayVoucher' ],
	//当创建一个控件的时候需要在此引入
	requires : [ 'pb.view.pay.PayVoucherQueryHunanBOC' ],
	//创建后获取当前控件，应用于当前控制层
	refs : [ {
			ref : 'query', //当前控制层引用
			selector : 'payVoucherQueryHunanBOC' // 控件的别名
	} ],
	onLaunch: function() {
		//当前控制层
		var me = this;
		...
		me.getStore('pay.PayVouchersHunanBOC').on('beforeload',function(thiz, options) {
			//查询区
			if (isExistStatus) {
				//当前状态控件
		...
		if (!isExistStatus) {
			Ext.PageUtil.onInitList(panel, 'pay.PayVouchersHunanBOC');
			if(initialLoad) {
				me.getStore('pay.PayVouchersHunanBOC').load();
			}
		} else {
			/**
			 * 初始化切换列表使用的store
			 */
			Ext.PageUtil.onInitPage(panel, _menu.statusList, 'pay.PayVouchersHunanBOC');
			/**
			 * 初始化页面
			 */
