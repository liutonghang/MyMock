 ...
 * @memberOf {TypeName} 
 */
Ext.define('pb.controller.pay.BPayVouchers', {
	extend : 'Ext.app.Controller',
	// 数据集列表：
	// pay.PayVouchers 支付凭证
	stores : [ 'pay.BPayVouchersHunanBOC'],
	//对象模型 pay.PayVoucher 支付凭证
	models : [ 'pay.APayVoucher' ],
	//当创建一个控件的时候需要在此引入
	requires : [ 'pb.view.pay.BPayVoucherQueryHunanBOC' ],
	//创建后获取当前控件，应用于当前控制层
	refs : [ {
			ref : 'query', //当前控制层引用
			selector : 'bPayVoucherQueryHunanBOC' // 控件的别名
	} ],
	onLaunch: function() {
		...
		me.getStore('pay.BPayVouchersHunanBOC').on('beforeload',function(thiz, options) {
			//查询区
		...
		if (!isExistStatus) {
			Ext.PageUtil.onInitList(panel, 'pay.BPayVouchersHunanBOC');
			if(initialLoad) {
				me.getStore('pay.BPayVouchersHunanBOC').load();
			}
		} else {
			/**
			 * 初始化切换列表使用的store
			 */
			Ext.PageUtil.onInitPage(panel, _menu.statusList, 'pay.BPayVouchersHunanBOC');
			/**
			 * 初始化页面
			 */
