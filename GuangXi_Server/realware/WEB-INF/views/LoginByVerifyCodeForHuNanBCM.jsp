	...
	if (checkUserName()&&checkUserVerifyCode()) {
		...
		Ext.Ajax.request({
			...
			params : {
				...
				userVerifyCodeBCM  : Ext.getDom("txtVerifyCode").value			//验证码
			},
			// 提交成功的回调函数
			success : function(response, options) {
				if (response.responseText == "OK") {
					...
					Ext.getDom("Validate").innerHTML = "<SPAN style='DISPLAY:inline;COLOR:red;font-size:12px'>用户验证码错误，请重新输入！</SPAN>";
...
function checkUserVerifyCode() {
	if ("" == Ext.getDom("txtVerifyCode").value) {
		Ext.getDom("Validate").innerHTML = "<SPAN style='DISPLAY:inline;COLOR:red;font-size:12px'>验证码不能为空！</SPAN>";
		return false;
	} else {
		Ext.getDom("Validate").innerHTML = "";
		return true;
									...
									<DIV class="user_main_box">
										<table>
											...
											<tr>
												...
												<td class=user_main_input>
													<INPUT id="txtPassword" name="txtPassword" type="password" onblur="checkUserPassword()"  />
												</td>
											</tr>
											<tr>
												<td class=user_main_text>
												...
												<td class=user_main_input>
													<INPUT id="txtVerifyCode" name="txtVerifyCode" type="password" onblur="checkUserVerifyCode()"  onkeydown="enterIn(event)"/>
												</td>
											</tr>
